...
SUITE(CantorSetTest) {
    ...
    TEST(TestSimple){
        CantorSet original(std::string("{1}"));
        CantorSet cantor= CantorSet::createCompleteCantorSet(original);
        CHECK_EQUAL("{1}",cantor.toString());
    }
    TEST(OPerSrav){
        CantorSet original(std::string("{1,2,3,4,5,6}"));
        CantorSet origin(std::string("{7,8}"));
        CHECK(origin>original);
        original+=origin;
        CHECK_EQUAL("{1, 2, 3, 4, 5, 6, 7, 8}",original.toString());
        CHECK_EQUAL("{7, 8}",(original*=origin).toString());
    }
    ...
    TEST(OperatorPotoka){
        CantorSet potok;
        std::istringstream test_input("{1,2,3}");
        auto orig_cin_buf = std::cin.rdbuf();
        std::cin.rdbuf(test_input.rdbuf());
        std::cin >> potok;
        std::cin.rdbuf(orig_cin_buf);
        CHECK_EQUAL("{1, 2, 3}",potok.toString());
        CantorSet origin("{1,2,3}");
        std::ostringstream output;
        output<<origin;
        CHECK_EQUAL("{1, 2, 3}",output.str());
    }
    TEST(LowLevel){
        CantorSet original(std::string("{1,2,3,4,5,6}"));
        CantorSet cantor = CantorSet::createCompleteCantorSet(original);
        CantorSet proverka = cantor.getLevel(2);
        CHECK_EQUAL("{1, 6}",proverka.toString());
    }
    TEST(DetailedString){
        CantorSet original(std::string("{1,2,3,4,5,6}"));
        CHECK_EQUAL("CantorSet: {1, 2, 3, 4, 5, 6}\nCardinality: 6\nTotal elements: 6\n",original.toDetailedString());
    }
